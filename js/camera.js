...
AFRAME.registerComponent('pinch-scale', {
    ...
    handleEvent: function (event) {
        ...
        if (gltfModel.object3D.scale.x <=2.5) {
        ...
        if (gltfModel.object3D.scale.y <=2.5) {
            gltfModel.object3D.scale.x = this.scaleFactor * this.initialScale.x;
            gltfModel.object3D.scale.y = this.scaleFactor * this.initialScale.y;
            gltfModel.object3D.scale.z = this.scaleFactor * this.initialScale.z;
        }

        if (gltfModel.object3D.scale.z <=2.5) {
            gltfModel.object3D.scale.x = this.scaleFactor * this.initialScale.x;
            gltfModel.object3D.scale.y = this.scaleFactor * this.initialScale.y;
            gltfModel.object3D.scale.z = this.scaleFactor * this.initialScale.z;
        }
         ...
         {
            gltfModel.object3D.scale.x = this.data.min * this.data.min;
            gltfModel.object3D.scale.y = this.data.min * this.data.min;
            gltfModel.object3D.scale.z = this.data.min * this.data.min;       
         }
    }
});
